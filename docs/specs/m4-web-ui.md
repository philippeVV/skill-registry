...
**Enterprise:** Same static output deployed to S3 + CloudFront. Only the
...
- `PUBLIC_STATS_URL` — OTEL stats endpoint for client-side fetching.
  Empty by default (OSS). Set in CI for enterprise deploys.
...
2. Push static output to `gh-pages` branch (OSS) or S3 (enterprise)
...
| Deployment (enterprise) | S3 + CloudFront |
