...
where the index and package artifacts live for both the OSS and enterprise
versions.
...
For the enterprise version, the same format is hosted on a private Git repo
...
- Private enterprise registry is purely a configuration change
...
- Public OSS repo means all example packages are public — intentional for OSS,
  enterprise uses private repo

**Neutral:**
- `skr` defaults to the OSS repo as its registry source; enterprise teams
